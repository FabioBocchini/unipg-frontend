...
const { Column } = Table
  ...
  render() {  
      ...
      const data = this.state.data.map(item => ({...item, dataesametext:moment(item.dataesame).format('DD/MM/YYYY'),professore:item.cognome + " " + item.nome}))
            ...
            <Column
                  ...
                  render={(text, record) => {
                   if( (record.statoesame)==="Waiting"){
                    return (<span>
                        <Button type="link" onClick={() => this.accettaEsame(record.id_corso)}>Accetta</Button>
                        <Divider type="vertical" />
                        <Button type="link" onClick={() => this.rifiutaEsame(record.id_corso)}>Rifiuta</Button>
                    </span>)
                  } return(
                        <span>Accettato</span>
                  )
